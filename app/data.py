...
try:
    from .config import licensed_data_dir, load_local_environment
except ImportError:
    from config import licensed_data_dir, load_local_environment
...
def resolve_licensed_data_dir(data_dir: Path | None = None) -> Path:
    """Return the local cache containing entitled Maycee Parquet files."""
    candidates: list[Path] = []
    if data_dir is not None:
        candidates.append(Path(data_dir))
    candidates.append(licensed_data_dir())
    ...
    raise FileNotFoundError(
        "Licensed Maycee Parquet data was not found. Run "
        "scripts/fetch_licensed_data.py or set MAYCEE_LICENSED_DATA_DIR.\n"
        f"Searched:\n{searched}"
    )
...
def _parquet_glob(data_dir: Path, table: str) -> str:
    ...
    raise FileNotFoundError(f"Missing licensed Parquet table: {table}")
...
def load_dashboard_tables(data_dir: Path | None = None) -> dict[str, pd.DataFrame]:
    """Load dashboard-ready tables from the local licensed Maycee cache."""
    source_dir = resolve_licensed_data_dir(data_dir)
    ...
    return {
        ...
        "returns": returns_df,
        "source": pd.DataFrame({"kind": ["licensed"]}),
    }
...
def enriched_transactions(tables: dict[str, pd.DataFrame] | None = None) -> pd.DataFrame:
    ...
    con = _connection(tables or load_dashboard_tables())
    return con.execute(
        """
        select
            t.transaction_id,
